...
const Navbar = () => {
  ...
  return (
    <>
      ...
      {/* Mobile Menu (Moved outside of motion.header to avoid transform container clipping) */}
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 bg-background z-[9999] lg:hidden flex flex-col"
          >
            {/* Header inside Mobile Menu */}
            <div className="flex items-center justify-between px-6 py-4 border-b border-white/10 relative z-50">
              <Logo variant="auto" size="md" />
              <button
                className="p-2 rounded-xl bg-white/5 border border-white/10 hover:bg-white/10 transition-colors focus:outline-none"
              ...
              {navItems.map((item, idx) => (
                ...
                >
                  {item.href ? (
                    <Link
                      to={item.href}
                      className="text-lg font-semibold text-foreground mb-3 block lg:hover:text-primary transition-colors"
                    >
                      {item.label}
                    </Link>
                  ) : (
                      ...
                      <div className="space-y-2 pl-4">
                        {item.children?.map((child) => (
                          <Link
                            key={child.href}
                            to={child.href}
                            className="block py-2 text-muted-foreground lg:hover:text-primary transition-colors link-underline"
                          >
                            {child.label}
                          </Link>
                        ))}
